...
namespace ExcuseAgent
{
	public partial class MainWindowController : MonoMac.AppKit.NSWindowController
	{
		...
		public ExcuseAgent eAgent { get; set; }
		...
		{
			eAgent.SetGameServer(servers [gamesComboBox.StringValue]);
			eAgent.StartAgent();
			eAgent.JoinGameServer((errorMsg) => { ErrorAlert(errorMsg); }, () => { TimeoutAlert(); });
			btnJoinGame.Enabled = false;
		...
		{
			InvokeOnMainThread(() => {
				lblAgentStatus.StringValue = eAgent.AgentInfo.status;
				lblAgentID.StringValue = "" + eAgent.AgentInfo.processId;
				lblGameStatus.StringValue = eAgent.AgentInfo.gameStatus;
				lblGameID.StringValue = "" + eAgent.AgentInfo.gameID;

				if (eAgent.AgentInfo.CommonAgentInfo != null) {
					lblAgentLocation.StringValue = "" + eAgent.AgentInfo.CommonAgentInfo.Location.X + ", " + eAgent.AgentInfo.CommonAgentInfo.Location.Y;
					lblAgentHealth.StringValue = "" + eAgent.AgentInfo.CommonAgentInfo.Points;
					lblAgentSpeed.StringValue = "" + eAgent.AgentInfo.CommonAgentInfo.Speed;
					lblAgentStrength.StringValue = "" + eAgent.AgentInfo.CommonAgentInfo.Strength;
					lblNumOfTicks.StringValue = "" + eAgent.NumOfTicks;
					lblNumOfExcuses.StringValue = "" + eAgent.NumOfExcuses;
				} else {
